...
## Golang valyala
GOLANG_VALYALA_VERSION=1.6.4
GOLANG_VALYALA_DOWNLOAD_URL=https://github.com/valyala/fastjson/archive/refs/tags/v$(GOLANG_VALYALA_VERSION).tar.gz
GOLANG_VALYALA_RPM_SPECFILE=$(RPM_SPECDIR)/golang-github-valyala-fastjson.spec
GOLANG_VALYALA_RPM_SRC_TGZ := $(RPM_SOURCEDIR)/fastjson-$(GOLANG_VALYALA_VERSION).tar.gz
GOLANG_VALYALA_SRPM_OUTFILE := $(RPM_SRCRPMDIR)/$(shell rpm -q --specfile rpmbuild/SPECS/golang-github-valyala-fastjson.spec --eval "%{_rpmfilename}" | grep golang-github-valyala-fastjson-1.6.4).rpm
GOLANG_VALYALA_PRISTINE_SPECFILE := $(PRISTINE_SPECDIR)/golang-github-valyala-fastjson.spec
...
rpm: $(PRISTINE_SPECFILE) $(GOLANG_ZEEBO_PRISTINE_SPECFILE) $(GOLANG_VALYALA_PRISTINE_SPECFILE)
	rpmbuild --define="_topdir $(PRISTINE_TOPDIR)" -bb $(GOLANG_VALYALA_PRISTINE_SPECFILE)
...
.PHONY: download
download: $(RPM_SRC_TGZ) $(GOLANG_ZEEBO_RPM_SRC_TGZ) $(GOLANG_VALYALA_RPM_SRC_TGZ)

.PHONY: srpms
srpm: $(SRPM_OUTFILE) $(GOLANG_ZEEBO_SRPM_OUTFILE) $(GOLANG_VALYALA_SRPM_OUTFILE)
...
$(GOLANG_VALYALA_RPM_SRC_TGZ): | $(RPM_SOURCEDIR)
	wget -O "$(GOLANG_VALYALA_RPM_SRC_TGZ)" "$(GOLANG_VALYALA_DOWNLOAD_URL)"
...
$(GOLANG_VALYALA_SRPM_OUTFILE): $(GOLANG_VALYALA_RPM_SRC_TGZ) $(GOLANG_VALYALA_RPM_SPECFILE) | $(RPM_RPMDIR) $(RPM_SOURCEDIR) $(RPM_SPECDIR) $(RPM_SRCRPMDIR) $(RPM_BUILDDIR)
	rpmbuild --define="_topdir $(RPM_TOPDIR)" -bs $(GOLANG_VALYALA_RPM_SPECFILE)

$(GOLANG_ZEEBO_SRPM_OUTFILE): $(GOLANG_ZEEBO_RPM_SRC_TGZ) $(GOLANG_ZEEBO_RPM_SPECFILE) | $(RPM_RPMDIR) $(RPM_SOURCEDIR) $(RPM_SPECDIR) $(RPM_SRCRPMDIR) $(RPM_BUILDDIR)
...
$(GOLANG_VALYALA_PRISTINE_SPECFILE): $(GOLANG_VALYALA_SRPM_OUTFILE) |  $(PRISTINE_RPMDIR) $(PRISTINE_SOURCEDIR) $(PRISTINE_SPECDIR) $(PRISTINE_SRCRPMDIR) $(PRISTINE_BUILDDIR)
	rpm --define="_topdir $(PRISTINE_TOPDIR)" --install $(GOLANG_VALYALA_SRPM_OUTFILE)
